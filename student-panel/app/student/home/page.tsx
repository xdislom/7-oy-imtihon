    ...
    <div className="p-[20px] md:p-[30px] flex flex-col gap-6">
      <div>
        <h2 className="text-[22px] font-bold text-gray-800 flex items-center gap-2 mb-4">
          Kumushlar: 1427{" "}
          <i className="fa-regular fa-gem text-gray-400 text-[18px]"></i>
        </h2>
        ...
        <div className="bg-white rounded-[12px] p-6 shadow-sm w-[320px]">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-2 cursor-pointer text-gray-700 font-[500] text-[15px]">
              Iyun 2026{" "}
              <i className="fa-solid fa-chevron-down text-[10px] text-gray-500 mt-0.5"></i>
            </div>
          ...
          <div className="grid grid-cols-7 mb-4 text-center">
            {daysOfWeek.map((day, i) => (
              <div key={i} className="text-gray-400 text-[12px] font-[500]">
                {day}
              </div>
            ))}
          </div>
            ...
            {dates.flat().map((date, index) => {
              ...
              return (
                <div key={date} className="relative flex justify-center items-center h-8">
                  <div
                    className={`flex justify-center items-center w-8 h-8 rounded-full text-[13px] text-gray-700 font-[500] cursor-pointer ${
                      isCurrent ? "border border-gray-400" : "hover:bg-gray-50"
                    }`}
                  >
                    {date}
                  </div>
